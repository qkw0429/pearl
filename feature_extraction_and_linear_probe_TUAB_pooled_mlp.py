"""
TUAB layer-wise feature extraction + linear probe (pooled feature + SimpleMLP head variant).

Same pipeline as feature_extraction_and_linear_probe_TUAB.py, but instead of
...
DATE = datetime.datetime.now().strftime('%y%m%d_%H%M%S') + '_TUAB_LAYER_WISE_POOLED_MLP_PROBE'
...
# TUAB(원본) 스크립트와 temp 경로가 겹치면, 이미 있는 (pooling 안 된) layer_*.pt를
# 그대로 "이미 추출됨"으로 오인해서 재사용해버리는 사고가 날 수 있어 경로를 분리한다.
TEMP_ROOT = f"/workspace/EEGPT/temp/TUAB_pooled_mlp/{NOTRAIN_LINEAR_OR_FINETUNE_OR_PEARL_LORA_OR_DORA_VERA_OR_DYNAMICPEARL}"
LOG_ROOT = f"/workspace/EEGPT/log/TUAB_layerwise_pooled_mlp_probe_log/{DATE}/{NOTRAIN_LINEAR_OR_FINETUNE_OR_PEARL_LORA_OR_DORA_VERA_OR_DYNAMICPEARL}_model/SEED_0"
...
if __name__ == "__main__":
    ...
    print(f"\nTUAB {NOTRAIN_LINEAR_OR_FINETUNE_OR_PEARL_LORA_OR_DORA_VERA_OR_DYNAMICPEARL} POOLED-MLP LAYER-WISE FEATURE EXTRACTION + LINEAR PROBE END")
