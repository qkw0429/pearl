"""
TUEV layer-wise feature extraction + linear probe (pooled feature + SimpleMLP head variant).

Same pipeline as feature_extraction_and_linear_probe_TUEV.py, but instead of
reproducing the real model's self.head (Dropout + LinearWithConstraint on the
full flattened (N_time_patch, EMBED_NUM, D) feature), this variant follows the
older EEGPT-style probing setup: mean-pool the feature over the time-patch and
summary-token axes (POOLING = [1, 2]) down to a single (B, D) embedding per
layer, then fit a simple MLP/linear head (SimpleMLP) on top — the way probing
was done before the classifier-head-matching StructuredProbe was introduced.
...
NOTRAIN_LINEAR_OR_FINETUNE_OR_PEARL_LORA_OR_DORA_VERA_OR_DYNAMICPEARL:
  1. Extract layer-wise features (pooled to (B, D)) for train / valid / test
     splits, caching them under a temp directory.
  2. Run a SimpleMLP probe on each of the 8 layers, one layer at a time.
...
from Modules.models.EEGPT_mcae_finetune_change_tuev_for_linear_wise_invest import EEGPTClassifier, EEGPTClassifier_rep_conv, EEGPTClassifier_rep_MULTI_SCALE_DYNAMIC_CONV
...
NOTRAIN_LINEAR_OR_FINETUNE_OR_PEARL_LORA_OR_DORA_VERA_OR_DYNAMICPEARL = 'NOTRAIN'

# 1, 2차원(N_time_patch, EMBED_NUM)을 mean pooling해서 (B, D) 임베딩으로 축소한다.
POOLING = [1, 2]
...
DATE = datetime.datetime.now().strftime('%y%m%d_%H%M%S') + '_TUEV_LAYER_WISE_POOLED_MLP_PROBE'
...
# SimpleMLP의 은닉층 차원 리스트. Linear probing이면 빈 리스트([])를 써서
# 입력을 바로 num_outputs로 매핑하는 순수 linear classifier가 되게 한다.
MLP_HIDDEN_LAYER_SIZES = []

# TUEV(원본) 스크립트와 temp 경로가 겹치면, 이미 있는 (pooling 안 된) layer_*.pt를
# 그대로 "이미 추출됨"으로 오인해서 재사용해버리는 사고가 날 수 있어 경로를 분리한다.
TEMP_ROOT = f"/workspace/EEGPT/temp/TUEV_pooled_mlp/{NOTRAIN_LINEAR_OR_FINETUNE_OR_PEARL_LORA_OR_DORA_VERA_OR_DYNAMICPEARL}"
LOG_ROOT = f"/workspace/EEGPT/log/TUEV_layerwise_pooled_mlp_probe_log/{DATE}/{NOTRAIN_LINEAR_OR_FINETUNE_OR_PEARL_LORA_OR_DORA_VERA_OR_DYNAMICPEARL}_model/SEED_0"
...
class TUEVLoader(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 파일명이 숫자(Train)로 시작하는지, 알파벳(Eval)으로 시작하는지에 따라
        # 파싱 로직이 달라지므로 파일명에서 첫 번째 숫자 묶음을 subject_id로 사용
...
def get_dataset(args):
    if args.dataset == 'TUEV':
        train_dataset, test_dataset, val_dataset = prepare_TUEV_dataset("/workspace/EEGPT/dataset/TUEV/processed")
        ch_names = ['EEG FP1-REF', 'EEG FP2-REF', 'EEG F3-REF', 'EEG F4-REF', 'EEG C3-REF', 'EEG C4-REF', 'EEG P3-REF', 'EEG P4-REF', 'EEG O1-REF', 'EEG O2-REF', 'EEG F7-REF', \
                    'EEG F8-REF', 'EEG T3-REF', 'EEG T4-REF', 'EEG T5-REF', 'EEG T6-REF', 'EEG A1-REF', 'EEG A2-REF', 'EEG FZ-REF', 'EEG CZ-REF', 'EEG PZ-REF', 'EEG T1-REF', 'EEG T2-REF']  # 안쓰임
...
def extract_and_merge_split(split_name, dataset, model, temp_dir, device, batch_size=EXTRACT_BATCH_SIZE):
    ...
    (배치 하나에 8개 layer를 한 dict로 묶어 파일 하나로 저장하면, merge 시 layer
    1개만 필요해도 torch.load가 파일 안의 8개 layer를 전부 역직렬화해야 한다.
    layer별로 파일을 완전히 분리하면 그 layer의 파일만 읽으면 되고 다른 layer의
    데이터는 전혀 건드리지 않는다.)
    POOLING=[1, 2]이므로 여기서 이미 (B, N_time_patch, EMBED_NUM, D) 대신
    (B, D)로 pooling된 상태로 저장된다.
...
# ==========================================
# 2. Linear probe (SimpleMLP on pooled (B, D) feature)
# ==========================================

class SimpleMLP(nn.Module):
    """
    EEGPT classifier head(self.head)를 그대로 재현하기 이전에 쓰던 방식: 입력
    feature를 pooling해서 얻은 (B, num_features) 임베딩 위에, hidden_layer_sizes가
    빈 리스트면 순수 linear classifier로, 하나 이상 채워져 있으면 ReLU가 있는
    간단한 MLP로 동작하는 head. 무작위 초기화 후 매번 새로 학습한다.
    """
    def __init__(
        self,
...
def load_layer_dataset(feature_path):
    ...
    TensorDataset으로 반환한다. POOLING=[1, 2]로 이미 (B, D) 형태로 pooling되어
    저장되어 있으므로 추가 처리는 필요 없다.
    """
...
def run_layer_experiment(
    ...
    _, num_features = train_dataset.tensors[0].shape
    print(f"[Layer {layer_num}] pooled feature shape = (num_features={num_features})")
    probe = SimpleMLP(
...
if __name__ == "__main__":
    ...
    print(f"\nTUEV {NOTRAIN_LINEAR_OR_FINETUNE_OR_PEARL_LORA_OR_DORA_VERA_OR_DYNAMICPEARL} POOLED-MLP LAYER-WISE FEATURE EXTRACTION + LINEAR PROBE END")
