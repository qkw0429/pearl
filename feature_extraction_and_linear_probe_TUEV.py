"""
TUEV layer-wise feature extraction + linear probe (integrated, single-run-per-method).
...
This merges the previous feature_extraction_TUEV.py and linear_probe.py
(originally written for BCIC-2A) into one pipeline; the probe/metric logic
follows the format of the BCIC-2A linear_probe.py, adapted for TUEV's
6-class ("multiclass", num_outputs=6) classification setting.
"""
...
if __name__ == "__main__":
    ...
    print(f"\nTUEV {NOTRAIN_LINEAR_OR_FINETUNE_OR_PEARL_LORA_OR_DORA_VERA_OR_DYNAMICPEARL} LAYER-WISE FEATURE EXTRACTION + LINEAR PROBE END")
